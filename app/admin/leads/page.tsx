...
export default function LeadsPage() {
  ...
  const mark = async (id: string, status: string) => {
    await updateStatus({ id: id as any, status, token: getStoredToken()! });
        ...
        {leads?.map((lead: any) => (
          <div key={lead._id} className="rounded-lg border border-border bg-card p-4">
            ...
            <div className="mt-2 flex gap-2">
              ...
              <button onClick={() => deleteLead({ id: lead._id, token: getStoredToken()! })} className="rounded bg-destructive/10 px-2 py-1 text-xs text-destructive">Delete</button>
            </div>
          </div>
        ))}
      </div>
